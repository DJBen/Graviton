...
public struct HorizonsQuery: Hashable {
    ...
    var rtsMode: RTSMode = .trueVisualHorizon
    ...
    public static func observerQuery(target: Naif, site: CLLocation, startTime: Date, stopTime: Date) -> HorizonsQuery {
        var query = HorizonsQuery(naif: target, tableType: .observer, startTime: startTime, stopTime: stopTime)
        query.site = site
        query.center = "coord@" + String(Naif.majorBody(.earth).rawValue)
        return query
    }
