...
class EphemerisTest: XCTestCase {
    ...
    override func setUp() {
        ...
        let bodies = [301, 399, 10, 499, 402, 599, 501, 509, 508].map { (id) -> CelestialBody in
            ...
            if id == 599 {
                body = CelestialBody(naifId: id, name: "blah", gravParam: 123, radius: 343, obliquity: DegreeAngle(12))
            } else {
    ...
    func testClosestBodyQuery() {
        ...
        let body3 = ephemeris.closestBody(toUnitPosition: Vector3(10, 10.1, 9.9).normalized().oblique(by: DegreeAngle(12)), from: ephemeris[599]!, maximumAngularDistance:RadianAngle(degreeAngle: DegreeAngle(5)))
